...
@OnlyIn(Dist.CLIENT)
public class ShockboltProjectileRenderer extends EntityRenderer<ShockboltProjectileEntity> {
    private static final ResourceLocation TEXTURE = new ResourceLocation("voxelmagica","textures/entity/spells/shockboltprojectile.png");
    private static final RenderType type = RenderType.getEntityCutoutNoCull(TEXTURE);

    public ShockboltProjectileRenderer(EntityRendererManager renderManagerIn) {
        super(renderManagerIn);
    }

    protected int getBlockLight(ShockboltProjectileEntity entityIn, float partialTicks) {
        return 15;
    }

    public void render(ShockboltProjectileEntity entityIn, float entityYaw, float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn) {
        matrixStackIn.push();
     ...
     */
    public ResourceLocation getEntityTexture(ShockboltProjectileEntity entity) {
        return TEXTURE;
    }
}
