...
public class SpellAeroblastItem extends Item {

    private static int manaCostFull = 600;
    private static int manaCostReduced = 250;
    private static int cooldown = Config.GCD.get();

    public SpellAeroblastItem(){
        super(new Properties().maxStackSize(1).group(VoxelMagica.creativeTab));
        setRegistryName("spellaeroblast");
    }
    ...
    public ActionResult<ItemStack> onItemRightClick(World worldIn, PlayerEntity playerIn, Hand handIn) {
        ...
        playerIn.getCapability(ManaProvider.MANA_CAP).ifPresent(m -> {
            ...
            if(m.getMana() >= manaCost || playerIn.isCreative()){
                ...
                EffectInstance blackMana = playerIn.getActivePotionEffect(ModEffects.WHITEMANA_EFFECT);
                int manaLevel = blackMana == null ? 0 : blackMana.getAmplifier() + 1;
                manaLevel = Math.min(manaLevel,4);
                EffectInstance newMana = new EffectInstance(ModEffects.WHITEMANA_EFFECT,12000,manaLevel,true,true);
                playerIn.addPotionEffect(newMana);


                if(!worldIn.isRemote()){
                    AeroblastProjectileEntity projectile = new AeroblastProjectileEntity(playerIn, worldIn);
                    ...
                    worldIn.playSound(null,playerIn.getPosX(),playerIn.getPosY(),playerIn.getPosZ(),
                            SoundEvents.ENTITY_BLAZE_SHOOT, SoundCategory.PLAYERS,.5f,1.7f);
                }
